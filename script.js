document.addEventListener('DOMContentLoaded', () => {
    ...
    function combinations(n, k) {
     ...
     async function fetchAndAnalyzeGames() {
        ...
        try {
            ...
            const historicalTimestamp = new Date(now.getTime() - 12 * 60 * 60 * 1000).toISOString().slice(0, 19) + 'Z';
    ...
    function createGameCard(game, index) {
        ...
        if (momentumResult.status === 'no_data') {
            ...
            momentumIndicatorHtml = `<span class="ml-2 flex items-center text-xs font-bold" style="color: ${shiftColor};" title="Momentum from 12hr line movement.">${arrowSvg}<span class="ml-1 mr-1">${momentumLabel}</span><span>${probabilityShift.toFixed(1)}%</span></span>`;
        }
