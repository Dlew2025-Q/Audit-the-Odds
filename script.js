document.addEventListener('DOMContentLoaded', () => {
    ...
    /**
     * Calculates momentum based on the change in implied probability over a time window.
     * @param {object} currentGame - The current game data object from the API.
     * @param {object} historicalGame - The historical game data object from the past.
    ...
    function createGameCard(game, index) {
        ...
        if (momentumResult.status === 'no_data') {
            ...
            momentumIndicatorHtml = `<span class="ml-2 flex items-center text-xs font-bold" style="color: ${shiftColor};" title="Momentum from 6hr line movement.">${arrowSvg}<span class="ml-1 mr-1">${momentumLabel}</span><span>${probabilityShift.toFixed(1)}%</span></span>`;
        }
