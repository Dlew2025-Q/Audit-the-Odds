document.addEventListener('DOMContentLoaded', () => {
    ...
    function createGameCard(game, index) {
        ...
        if (momentumResult.status === 'no_data') {
            momentumIndicatorHtml = `<span class="ml-2 flex items-center text-xs font-bold" style="color: var(--text-secondary);" title="Point spread data not available for momentum calculation."><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" /></svg><span>N/A</span></span>`;
        } else {
            ...
            momentumIndicatorHtml = `<span class="ml-2 flex items-center text-xs font-bold" style="color: ${shiftColor};" title="Momentum adjustment based on the market point spread.">${arrowSvg}<span class="ml-1 mr-1">${momentumLabel}</span><span>${probabilityShift.toFixed(1)}%</span></span>`;
        }
    ...
    function applyFiltersAndSorting() {
        ...
        buildMomentumParlayBtn.disabled = !ALL_SPORTS_DATA.some(game => game.spread_away != null && game.spread_home != null);
    ...
    function handleBuildMomentumParlay() {
        ...
        ALL_SPORTS_DATA.forEach((game, index) => {
            ...
            const normalizedAwayProb = (1 / game.moneyline_away) / (1 / game.moneyline_away + 1 / game.moneyline_home) || 0.5;
            const momentumResult = getMomentumAdjustedProbability(normalizedAwayProb, game);
            if (momentumResult.status === 'no_data') return;
            const probabilityShift = (momentumResult.prob - normalizedAwayProb) * 100;
